...
While we wait, we have plenty of MEGA65 projects to dive into. I've shared earlier how to [setup a MEGA65 on a Nexys4](/mega65-nexys) for less than $300 and [install the free Xemu emulator on a Mac](/xemu-on-mac).

In these projects, I mention the closed ROM for the MEGA65 is available to purchasers of a [MEGA65](https://www.mega65.org) or [Dev Kit](/mega65-1) and those who've not purchased a Dev Kit must "technically/legally" use the [open ROM](https://files.mega65.org?id=8aec2fba-3b0a-4677-80ae-7a7f5f4f0cb8); which is not yet ready for prime time.
...
2. Click the `v` on the right side of the page. The RDF and BDF version history will display as shown in the image below:
    ...
    ![RDF and BDF version history for ROM Patch](/images/mega65/rdf-version-history.png)
    
[2022-05-15] Dan Sanderson posted a comment below noting changes since the original post. I have noted the changes.
[2024-01-03] Ton Def emailed to let me know that the reference ROM is now 910828. I have updated this blog post to include this updated information.
...
3. [Install the MEGA65 on a Nexys4 or A7 FPGA \| Build your own MEGA65!](/mega65-nexys)
...
2. Search for ~~`911001.BIN`~~ [UPDATE: 2022-05-15] `910828.BIN`.

    > **NOTE:** The file name of the `.BIN` file is the date of the last official ROM; October 1, 1991.

3. If you find a page that has multiple C65 ROMs, download one with the specific name listed in step 2.
4. Make a copy of the file with the name ` 910828_archive.BIN` in case you need a copy of the original ROM file.
5. Place the `910828.BIN` file on your desktop or in another easy to find location.
...
Xemu uses the Xmega65 emulator. I've shared how to set up and use Xemu on a Mac [in an earlier post](/xemu-on-mac); however, I will again share the process to use the original C65 ROM file before I test it:
...
4. [UPDATE: 2022-05-15] From the menu, *Disks* \| *SD-card* \| *Update files on SD image* . The *select your ROM image file* window will appear as shown in the image below:
    ...
    ![xMEGA Menus](/images/mega65/xemu-menu-update-sd-card.png)
...
7. Click the *OK* button and *xmega65* will boot, as shown in the image below:
    ...
    ![*xmega65* with MEGA65 licensed ROM](/images/mega65/c65-rom.png)
...
2. From the main menu, select *File* \| *Patch ROM File* and the Patch ROM file dialog box will appear as shown below:
    ...
    ![Patch ROM File dialog box](/images/mega65/patch-rom-dialog.png)
...
4. Navigate to the location of the original C65 ROM file named `910828.BIN` and select.
...
2. I should have shared this technique immediately after my posts on the [Xemu emulator](/xemu-on-mac) or [Nexys4 FPGA](/mega65-nexys4-livestream). Apologies to those frustrated using the open ROM option on these platforms.

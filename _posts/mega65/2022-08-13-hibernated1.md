...
Title: _Unbox and Play Hibernated 1 (Director's Cut) for MEGA65
_
...
I may not have been familiar with Hibernated 1, but I played text adventures in the 1980s and loved every journey. And as someone who loves to draw, I created maps on grid paper of the adventures to document the areas and items. I'm not sure the number of hours I burned playing these games, but can tell you, I had great adventures! When the box arrived, I couldn't wait to dive into the world of Hibernated 1 and I knew I would share my first experience with the game during a livestream.

## Start the Game

After I unboxed the game, I started my adventure and the community in chat wanted to assist!

To start the game, mount the .d81 disk image from the SD card using the Freezer or insert the floppy into the MEGA65. If you use the floppy, type the `MOUNT` command to attach the floppy to drive 8, unless you have the floppy set as your default media in the MEGA65 configuration.

To start Hibernated 1, type `BOOT`. A graphic intro screen will load. Hit `⏎` or `␣`, watch the loader do its work, and then begin your adventure. The graphic screen intro is new to the director's edition and takes full advantage of the advanced graphics capability of the MEGA65. It’s a nice touch and I look forward to future text adventures for the MEGA65 that include in-game images or even a full-fledge [SCUMM](https://en.wikipedia.org/wiki/SCUMM)-based point and click adventure game. It's more work to make these conversions, but the result would be fabulous!
...
Instead of `north`, `south`, `east`, and `west`, the game instructions explain that these are not useful in space. Instead, you are to use nautical directions `port`, `starboard`, `fore`, and `aft`.
...
Bitshifter, who helped convert the Z-Machine engine to the MEGA65, saw my livestream and send me an email:
...
The `save` and `restore` functions were the first issue. Using these commands would cause the MEGA65 to hang. I emailed Stefan, and to my surprise he copied Bitshifter, a MEGA65 developer. Less than 24 hours later, Bitshifter identified the error, and issued a fix. He substituted a `JSR` machine code instruction to a `JMP` in the interpreter.

The next day I found another error. Using the `restore` command with a filename not found on the disk caused the software to crash and display a garbled screen of characters. I sent the error to Bitshifter and he issued another fix. Stefan then updated the disk image download. If you have the boxed software and want to update your floppy disk, I share this process in the livestream.
...
Member of my community have asked if I will fire up another livestream and play more Hibernated 1. I'm considering it. I had a blast playing a text adventure over 30 years later. Including the folks in chat during the livestream made it more amusing. I won't finish the game during a livestream. That will take away the surprise for others and could harm sales of the game. But I am eager to once again enter the world of Hibernated 1 and look forward to the sequel coming later in 2022.

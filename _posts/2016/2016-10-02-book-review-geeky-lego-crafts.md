...
The book wastes no time presenting the models. The intro is short and followed by a single contents page listing all 21 builds. With titles like…
...
Each project begins with a short description, the build difficulty level (using a creative brick bar graph), and the elements necessary. Below is the first page of the *cassette letter holder* as an example.

![][image-2]

Next comes the build instructions. Unlike some build instructions that note which bricks to use for each step, No Starch and David use a system I've not seen frequently. As shown in the sample *mounted deer* build image below, new elements are opaque and existing elements are transparent. 
...
# Build style
It is obvious that David draws from his art and cartooning background with these builds. I describe these builds as a mix of 3D and 2D mosaics. The build style is heavily pixelated and "retro." I'm a huge fan of [Chris McVeigh's][5] work and these models are the perfect introduction to quirky and geeky builds.
